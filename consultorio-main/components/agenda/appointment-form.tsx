...
export function AppointmentForm({ missionaries }: Props) {
  ...
  async function handleSubmit(formData: FormData) {
    try {
      ...
      setError(e instanceof Error ? e.message : 'Error al agendar. Intente nuevamente.')
    }
  }
            ...
            <form action={handleSubmit} className="flex flex-col gap-3">
              ...
              <div>
                ...
                <input
                  name="reason"
                  type="text"
                  placeholder="Ej: Seguimiento, consulta de rutina..."
                  className="w-full rounded-lg px-3 py-3 text-base outline-none focus:ring-2 focus:ring-violet-500"
                  style={{ background: 'var(--bg-overlay)', color: 'var(--text)', border: '1px solid var(--border)' }}
                />
              </div>
